...
createNode = ()->
	...
	# 配置内容
	title = (if @config['title'] isnt false then '<div class="tan-title"><span>' + @config['title'] + '</span><i class="close-this fa fa-close"></i></div>' else '')
...
#提示框
window.tanAlert = (msg, fn, enter, leave)->
	new tan(
		skin:'tanAlert'
		title : '提示'
		width : 300
...
#提示确认框
window.tanConfirm = (msg, ok, cancel)->
	new tan(
		skin:'tanConfirm'
		title : '提示'
		width : 300
...
window.errorTips = (msg, time, fn, top) ->
	tanTips(msg, time, fn, 'd', top).show(no)

window.sucTips = (msg, time, fn, top) ->
	tanTips(msg, time, fn, 's', top).show(no)

window.infoTips = (msg, time, fn, top) ->
	tanTips(msg, time, fn, 'i', top).show(no)

window.warnTips = (msg, time, fn, top) ->
	tanTips(msg, time, fn, 'w', top).show(no)

window.primaryTips = (msg, time, fn, top) ->
